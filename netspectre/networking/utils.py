...
import netspectre.console.shell as shell
from netspectre.common.globals import BIN_TC, BIN_IPTABLES, BIN_SYSCTL, IP_FORWARD_LOC
...
def get_mac_by_ip(interface, address):
    """
    Resolves hardware address from IP by checking system ARP cache first,
    then fallback to sending an ARP request
    """
    # 1. Try reading from system ARP cache (/proc/net/arp)
    try:
        with open("/proc/net/arp", "r") as f:
            for line in f.readlines()[1:]:
                parts = line.split()
                if len(parts) >= 6 and parts[0] == address and parts[5] == interface:
                    mac = parts[3]
    ...
    # 2. Fallback to sending ARP request via Scapy
